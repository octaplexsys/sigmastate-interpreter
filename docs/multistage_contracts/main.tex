...
%particular, if we include more than the UTXOable to support transaction trees by putting the entire spending transaction into the execution context, so that we can encode predicates about other inputs and outputs of the transaction. 
